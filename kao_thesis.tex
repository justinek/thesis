...
In addition, we assume that the listener has prior knowledge that the \textsc{immune system} is related to the entities \textsc{body} and \textsc{virus} in some unknown way, as well as prior knowledge that \textsc{virus} harms \textsc{body}\footnote{We believe it is reasonable to assume that listeners need some prior knowledge regarding the more obvious relationships held in the target domain in order to fully understand these types of relational metaphors. Future work should empirically test the kinds of prior knowledge required to generate appropriate interpretations and inferences for novel relational metaphors.}. However, the listener is uncertain about which relations hold between \textsc{immune system} and the other two entities. The listener's hypotheses about the target domain can be represented by the eight possible labeled directed graphs shown in Figure~\ref{target-graph}, where \textsc{virus} harms \textsc{body}. An interpretation of the metaphor can thus be represented as a distribution over these eight graphs $G=(N, E)$, where $N$ is an ordered set of labeled nodes, and $E$ is an ordered set of labeled directed edges.
...
\node[main node] (A)                    {IS};
  ...
  \path (A) edge  [above left =1cm and 0.5cm]             node {protects} (B)
           (A) edge [above]             node {fights} (C)
          (B) edge              node {harms} (C);
          
   \end{scope}

   % graph6
     \begin{scope}[yshift=-3.5cm, xshift=4cm]
\node[main node] (A)                    {IS};
  \node[main node]         (B) [above right = 1.2cm and 0.5cm of A] {virus};
  \node[main node]         (C) [below right = 1.2cm and 0.5cm of B] {body};

  \path (B) edge  [above left =1cm and 0.5cm]             node {protects} (A)
           (A) edge [above]             node {fights} (C)
          (B) edge              node {harms} (C);
   ...
   % graph7
     \begin{scope}[yshift=-3.5cm, xshift=8cm]
\node[main node] (A)                    {IS};
  \node[main node]         (B) [above right = 1.2cm and 0.5cm of A] {virus};
  \node[main node]         (C) [below right = 1.2cm and 0.5cm of B] {body};

  \path (A) edge  [above left =1cm and 0.5cm]             node {protects} (B)
           (C) edge [above]             node {fights} (A)
          (B) edge              node {harms} (C);
          
   \end{scope}
   
      % graph8
     \begin{scope}[yshift=-3.5cm, xshift=12cm]
\node[main node] (A)                    {IS};
  \node[main node]         (B) [above right = 1.2cm and 0.5cm of A] {virus};
  \node[main node]         (C) [below right = 1.2cm and 0.5cm of B] {body};
...
\caption{All possible labeled directed graphs that describe the target domain \textsc{immune system} (IS), given that the relation between \textsc{body} and \textsc{virus} is unknown, and given uncertainty about the other two pair-wise relations.}
\label{target-graph}
\end{figure}
